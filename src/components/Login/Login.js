...
const Login = (props) => {
  ...
  const emailChangeHandler = (event) => {
    ...
    setFormIsValid(
      event.target.value.includes("@") && enteredPassword.trim().length > 6 // 判断一下form的validity更新form状态
    );
  };
  ...
  const validatePasswordHandler = () => {
    setPasswordIsValid(enteredPassword.trim().length > 6); //检查并更新psw合法性
  };

  const submitHandler = (event) => {
    event.preventDefault();
    props.onLogin(emailState.value, enteredPassword); //上交最新email和psw状态
  };
